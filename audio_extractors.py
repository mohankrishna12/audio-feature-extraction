...
#whole_file_features = extract_features("AudioSnippets/snippet0.wav")

#print(whole_file_features.shape)
...
def get_filtered(curr_file):
	filtered_features = pd.DataFrame()
			
	
	file = curr_file
	filter_directory = "AudioFilter/"

	loc = generate_file_name(filter_directory, file, 20, 250)
	essentia_features = extract_essentia_features(loc) 
	extract_freq_band(file, 20, 250, loc)
...
#audiofile = "AudioSnippets/snippet0.wav"

def getFeatures(audiofile):
	description = pd.DataFrame({
			#'Unnamed: 0': [0],
			'id':[audiofile.split('/')[-1].split('.')[0]],
			'target':[0]
		})

	filterfeatures = get_filtered(audiofile)
	#print('here')
	#print(filterfeatures)
	if(isinstance(filterfeatures,str)):
		return 'silent'
	allFeatures = pd.concat((description,extract_features(audiofile), filterfeatures), axis=1)
	print(allFeatures)
